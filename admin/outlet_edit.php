...
$query = 'SELECT outlet.*, user.nama_user,user.role,user.id_user FROM outlet LEFT JOIN user ON user.id_user = outlet.id_owner WHERE id_outlet = ' . stripslashes($_GET['id']);
$data = ambilsatubaris($conn, $query);

$query2 = 'SELECT user.*,outlet.nama_outlet FROM outlet RIGHT JOIN user ON user.id_user = outlet.id_owner WHERE user.role = "owner" order by user.outlet_id asc';
...
if (isset($_POST['btn-simpan'])) {
    ...
    $idownernew = stripslashes($_POST['owner_id_new']);
    $idowner = stripslashes($_POST['owner_id']);


    $query = "UPDATE outlet SET nama_outlet = '$nama' , alamat_outlet = '$alamat' , telp_outlet='$telp', id_owner = '$idowner' WHERE id_outlet = " . stripslashes($_GET['id']);


    if ($_POST['owner_id_new']) {
        $query1 = "UPDATE outlet SET nama_outlet = '$nama' , alamat_outlet = '$alamat' , telp_outlet='$telp', id_owner = '$idownernew' WHERE id_outlet = " . stripslashes($_GET['id']);
        ...
        <div class="col-md-12 col-lg-12 col-sm-12 col-xs-12">
            <div class="white-box">
                <form method="post" action="">
                    ...
                    <?php else : ?>
                        <div class="form-group">
                            <label>Owner Sekarang : <?php if ($data['role'] == 'owner') {
                                                        echo htmlspecialchars($data['nama_user']);
                                                    } ?></label>
