...
$q = 'SELECT *, SUM(total_harga) as total_harga FROM detail_transaksi INNER JOIN transaksi ON transaksi.id_transaksi = detail_transaksi.transaksi_id INNER JOIN member ON member.id_member = transaksi.member_id WHERE transaksi_id = ' . $_GET['id'];
$data = ambilsatubaris($conn, $q);
