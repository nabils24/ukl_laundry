...
if ($_GET['idtran']) {
    // get kode_invoice from table transaksi where id_transaksi $_GET['idtran']
    $q = ambilsatubaris($conn, 'SELECT kode_invoice from transaksi WHERE id_transaksi = ' . $_GET['idtran']);
    $invoice = $q['kode_invoice'];
} else {
...
if (isset($_POST['btn-simpan'])) {
    ...
    if ($_GET['idtran']) {
        $idtran = $_GET['idtran'];
        $query = "UPDATE transaksi SET member_id = '$member_id', tgl = '$tgl_sekarang', batas_waktu = '$batas_waktu', status = 'baru', status_bayar = 'belum', user_id = '$user_id' WHERE id_transaksi = '$idtran'";
        $execute = bisa($conn, $query);
    } else {
        $query = "INSERT INTO transaksi (outlet_id,kode_invoice,member_id,tgl,batas_waktu,biaya_tambahan,diskon,pajak,status,status_bayar,user_id) VALUES ('$outlet_id','$kode_invoice','$member_id','$tgl_sekarang','$batas_waktu',0,0,0,'baru','belum','$user_id')";
        $execute = bisa($conn, $query);
    }
    ...
    if ($execute == 1) {
        ...
        $sqlDetail = "INSERT INTO detail_transaksi (transaksi_id,id_member,paket_id,qty,total_harga) VALUES ('$transaksi_id','$member_id','$paket_id','$qty','$total_harga')";

        $executeDetail = bisa($conn, $sqlDetail);
        if ($executeDetail == 1) {
            header('location: transaksi_checkout.php?uid=' . $member_id . '&idtran=' . $transaksi_id . '&invoice=' . $invoice);
        } else {
                    ...
                    <div class="form-group">
                        ...
                        <select name="paket_id" class="form-control">
                            <?php foreach ($paket as $key) : ?>
                                <option value="<?= $key['id_paket'] ?>"><?= $key['nama_paket'] . " - " . $key['jenis_paket'];  ?></option>
                            <?php endforeach ?>
                        </select>
                    </div>
                    <div class="form-group">
